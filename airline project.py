...
if __name__ == '__main__':
    ...
    join=takeOff.join(landing,takeOff.src_airport==landing.dest_airport,"inner")

    maxAirport=join.select("src_airport",lit(col("takeoff") + col("landing")).alias("total")).orderBy(col("total").desc())\
    ...
    minAirport = join.select("src_airport", lit(col("takeoff") + col("landing")).alias("total")).orderBy(col("total").asc()) \
        .withColumn("rank", rank().over(Window.orderBy(col("total").asc()))).filter(col("rank") == 1).select(col("src_airport"))
    print("Q.3 ans:-")
    airportDf.join(minAirport, airportDf.IATA == minAirport.src_airport, "inner").drop("src_airport").show()
